      ...
      <div className="flex justify-center items-center gap-12 md:gap-24 px-6 flex-wrap">
        {CLIENTS.map((client, idx) => (
          <div key={idx} className="flex flex-col items-center space-y-4 shrink-0">
            <div className="relative w-24 h-24 md:w-32 md:h-32 rounded-full border border-white/10 bg-white/5 backdrop-blur-[20px] flex items-center justify-center transition-all duration-500 hover:border-white/50 hover:shadow-[0_0_20px_rgba(255,255,255,0.6)] cursor-default group/item overflow-hidden">
              {client.imageUrl ? (
                <img 
                  src={client.imageUrl} 
                  alt={client.name}
