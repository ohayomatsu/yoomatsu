...
export function Portfolio() {
  ...
  return (
    <section id="portfolio" className="py-24 px-6 max-w-7xl mx-auto space-y-16">
      ...
      <Tabs defaultValue="all" className="w-full flex flex-col items-center space-y-12" onValueChange={setActiveCategory}>
        <TabsList className="glass-effect p-1 h-auto rounded-full bg-white/5 border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
          {filteredProjects.map((project) => {
            ...
            return (
              <div key={project.id} className="group relative glass-card overflow-hidden aspect-video cursor-pointer">
                {image && (
                  <Image
                    src={image.imageUrl}
                    alt={project.title}
